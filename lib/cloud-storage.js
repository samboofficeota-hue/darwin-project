...
async function uploadSingleChunk(chunk, userId, sessionId) {
  try {
    console.log(`Getting signed URL for chunk ${chunk.id}...`);
    
    // 署名付きURLを取得
    const signedUrl = await getSignedUploadUrl(userId, sessionId, chunk.id);
    console.log(`Got signed URL for chunk ${chunk.id}: ${signedUrl.substring(0, 100)}...`);
    
    // ファイルをアップロード（タイムアウト付き）
    console.log(`Uploading chunk ${chunk.id} (${chunk.data.length} bytes)...`);
