...
export async function splitIntoHourlyFiles(audioFile, onProgress = null, sessionId = null) {
  ...
  console.log(`Recommended: Split into ${totalSegments} segments of 30 minutes each`);
  console.log(`Session ID: ${currentSessionId}`);
  
  // ユーザーに手動分割を案内するためのダミーファイルを作成
  const hourlyFiles = [];
  ...
  for (let i = 0; i < totalSegments; i++) {
    ...
    console.log(`Preparing segment ${i + 1}/${totalSegments}: ${startTime}s - ${endTime}s`);
    ...
    if (onProgress) {
      onProgress({
        ...
        message: `Preparing segment ${i + 1}/${totalSegments}`
      });
    }
    ...
    const fileName = `${baseName}_${currentSessionId}_segment_${segmentIndex.toString().padStart(3, '0')}.mp3`;
    
    // ダミーファイルを作成（実際の分割はユーザーが手動で行う）
    const dummyBlob = new Blob([''], { type: 'audio/mp3' });
    
    hourlyFiles.push({
      file: new File([dummyBlob], fileName, { type: 'audio/mp3' }),
      startTime,
      endTime,
