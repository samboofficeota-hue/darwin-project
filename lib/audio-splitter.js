...
export async function splitIntoHourlyFiles(audioFile, onProgress = null, sessionId = null) {
  ...
  console.log(`Splitting ${analysis.durationHours.toFixed(1)} hour file into ${totalSegments} segments of 30 minutes each`);
  ...
  for (let i = 0; i < totalSegments; i++) {
    ...
    try {
      // MediaRecorder APIを使用してシンプルに分割
      const segmentBlob = await recordAudioSegment(audioFile, startTime, actualDuration);
      
      // ファイル名を生成（連番を明確に定義）
      const baseName = audioFile.name.replace(/\.[^/.]+$/, '');
      const segmentIndex = i + 1; // 1から始まる連番
      const fileName = `${baseName}_${currentSessionId}_segment_${segmentIndex.toString().padStart(3, '0')}.webm`;
      
      hourlyFiles.push({
        file: new File([segmentBlob], fileName, { type: 'audio/webm' }),
        startTime,
        endTime,
...
/**
 * MediaRecorder APIを使用して音声の指定部分を録音
 * @param {File} audioFile - 音声ファイル
 * @param {number} startTime - 開始時間（秒）
 * @param {number} duration - 長さ（秒）
 * @returns {Promise<Blob>} 録音された音声データ
 */
async function recordAudioSegment(audioFile, startTime, duration) {
