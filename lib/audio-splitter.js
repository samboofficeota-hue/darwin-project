...
/**
 * 1時間ごとにファイルを分割してダウンロード
 * @param {File} audioFile - 音声ファイル
 * @param {Function} onProgress - 進捗コールバック関数
 * @returns {Promise<Array>} 分割されたファイルの配列
 */
export async function splitIntoHourlyFiles(audioFile, onProgress = null) {
  const analysis = await analyzeAudioFile(audioFile);
  const hourlyDuration = 3600; // 1時間 = 3600秒
  const totalHours = Math.ceil(analysis.duration / hourlyDuration);
  
  console.log(`Splitting ${analysis.durationHours.toFixed(1)} hour file into ${totalHours} hourly segments`);
  
  const hourlyFiles = [];
  
  for (let i = 0; i < totalHours; i++) {
    const startTime = i * hourlyDuration;
    const endTime = Math.min((i + 1) * hourlyDuration, analysis.duration);
    const actualDuration = endTime - startTime;
    
    console.log(`Creating hourly segment ${i + 1}/${totalHours}: ${startTime}s - ${endTime}s`);
    
    if (onProgress) {
      onProgress({
        current: i + 1,
        total: totalHours,
        percentage: Math.round(((i + 1) / totalHours) * 100),
        message: `Creating hourly segment ${i + 1}/${totalHours}`
      });
    }
    
    // 1時間分の音声データを抽出
    const segmentData = await extractAudioChunk(audioFile, startTime, actualDuration);
    
    // WAVファイルとしてダウンロード用のBlobを作成
    const wavData = base64ToArrayBuffer(segmentData);
    const blob = new Blob([wavData], { type: 'audio/wav' });
    
    // ファイル名を生成
    const baseName = audioFile.name.replace(/\.[^/.]+$/, '');
    const fileName = `${baseName}_hour_${i + 1}.wav`;
