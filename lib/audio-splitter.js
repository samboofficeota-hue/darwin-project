...
export async function splitIntoHourlyFiles(audioFile, onProgress = null, sessionId = null) {
  ...
  const segmentDuration = 900; // 15分 = 900秒（メモリ制限を考慮）
  ...
  console.log(`Splitting into ${totalSegments} segments of 15 minutes each`);
  ...
  for (let i = 0; i < totalSegments; i++) {
    ...
    try {
      ...
      // エラーが発生した場合は、より小さなセグメントで再試行
      if (error.message.includes('Segment too large')) {
        console.log(`Retrying segment ${i + 1} with smaller duration...`);
        try {
          // 半分の時間で再試行
          const retryDuration = actualDuration / 2;
          const segmentBlob = await extractAudioSegment(audioFile, startTime, retryDuration);
          ...
          hourlyFiles.push({
            file: new File([segmentBlob], fileName, { type: 'audio/wav' }),
            startTime,
            endTime: startTime + retryDuration,
          ...
          console.log(`Successfully created segment ${i + 1} with reduced duration`);
        } catch (retryError) {
          console.error(`Retry failed for segment ${i + 1}:`, retryError);
          continue;
        }
      } else {
        continue;
  ...
  return new Promise((resolve, reject) => {
    ...
    audio.addEventListener('loadeddata', async () => {
      try {
        ...
        if (estimatedSize > 500 * 1024 * 1024) { // 500MB制限に緩和
...
export async function splitAudioFile(audioFile, chunkDurationSeconds = 180, onProgress = null) {
  try {
    console.log('=== splitAudioFile START ===');
    ...
    // ファイルサイズチェック（100MB以上の場合は警告）
    if (audioFile.size > 100 * 1024 * 1024) {
      console.warn('Large file detected, may cause memory issues:', audioFile.size);
      throw new Error('ファイルサイズが大きすぎます（100MB以上）。先に時間分割を実行してください。');
    }
    ...
    // チャンク数が多すぎる場合の制限
    if (totalChunks > 50) {
      throw new Error(`チャンク数が多すぎます（${totalChunks}個）。チャンクサイズを大きくするか、先に時間分割を実行してください。`);
    }
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      // チャンクの音声データを抽出
      console.log(`Extracting chunk ${i + 1}/${totalChunks}...`);
      const chunkData = await extractAudioChunk(audioFile, startTime, actualDuration);
      console.log(`Chunk ${i + 1} extracted successfully, size: ${chunkData.length} chars`);
      ...
      // メモリ解放のための短い待機とガベージコレクション
      if (i < totalChunks - 1) {
        console.log(`Memory cleanup after chunk ${i + 1}/${totalChunks}`);
        await new Promise(resolve => setTimeout(resolve, 500)); // 少し長めに待機
        ...
        if (performance.memory) {
          ...
          console.log(`Memory usage: ${Math.round(memInfo.usedJSHeapSize / 1024 / 1024)}MB / ${Math.round(memInfo.totalJSHeapSize / 1024 / 1024)}MB`);
          
          // メモリ使用量が高い場合は警告
          if (memInfo.usedJSHeapSize > memInfo.totalJSHeapSize * 0.8) {
            console.warn('High memory usage detected, may cause issues');
          }
        }
      }
    }
...
async function extractAudioChunk(audioFile, startTime, duration) {
  return new Promise((resolve, reject) => {
    console.log(`=== extractAudioChunk START: ${startTime}s-${startTime + duration}s ===`);
    const audio = new Audio();
    const url = URL.createObjectURL(audioFile);
    ...
    audio.addEventListener('loadeddata', async () => {
      ...
      try {
        ...
        console.log('AudioContext created, fetching audio data...');
        const response = await fetch(url);
        const arrayBuffer = await response.arrayBuffer();
    ...
    audio.addEventListener('error', (error) => {
      console.error('Audio loading error in extractAudioChunk:', error);
      URL.revokeObjectURL(url);
      reject(new Error(`音声ファイルの読み込みに失敗しました: ${error.message || 'Unknown error'}`));
    });
