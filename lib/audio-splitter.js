 ...
 * ブラウザ上で音声ファイルを分割し、MP3形式でBase64データとして返す
 */
...
// WAVをMP3に変換
async function convertWavToMp3(wavData, sampleRate = 44100) {
  ...
  await ffmpeg.writeFile(inputFileName, new Uint8Array(wavData));
  ...
  return new Promise((resolve, reject) => {
    ...
    audio.addEventListener('loadeddata', async () => {
      try {
        ...
        // MP3に変換
        const mp3Data = await convertWavToMp3(wavData, audioBuffer.sampleRate);
        const base64Data = arrayBufferToBase64(mp3Data);
